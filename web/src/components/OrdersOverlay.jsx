...
export default function OrdersOverlay(){
  const [show,setShow]=useState(false), [orders,setOrders]=useState([]);
  useEffect(()=>{ const has=!!document.querySelector('[data-order-item],.order-card,[data-orders-list]');
    setShow(!has);
    const obs=new MutationObserver(()=>{ const now=!!document.querySelector('[data-order-item],.order-card,[data-orders-list]'); setShow(!now); });
    obs.observe(document.body,{childList:true,subtree:true}); return ()=>obs.disconnect();
  },[]);
  useEffect(()=>{ if(!show) return; (async()=>{ const eps=["/api/my/orders","/api/orders?scope=me","/api/orders"];
    for(const u of eps){ try{ const d=await j(u); setOrders(d.orders||d||[]); return; }catch{} } setOrders([]); })();
  },[show]);
  if(!show) return null;
  return (<div className="max-w-3xl mx-auto px-3 py-4" data-orders-list>
    ...
    {orders.length===0 ? <div className="text-zinc-400">Noch keine Bestellungen.</div> : (
      <div className="space-y-3">
        {orders.map(o=>{ const items=normItems(o);
          return (<div key={o.id||o.order_id} className="order-card p-3 rounded-xl bg-zinc-900/60 border border-zinc-800">
